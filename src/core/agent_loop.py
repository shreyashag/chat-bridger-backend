...
class AgentLoop:
    ...
    async def run_agent_stream(
        ...
        if client_tools:
            ...
            agent_with_tools.tools = list(agent_with_tools.tools or []) + client_function_tools
            ...
            logger.info(f"Added {len(client_function_tools)} client tools to agent with stop behavior")
    ...
    async def continue_after_client_tools(
        ...
        for item in session_items:
            ...
            if (item_dict.get("role") == "tool" and 
                isinstance(item_dict.get("content"), str) and 
                "PENDING_CLIENT_EXECUTION" in item_dict.get("content", "")):
                ...
                for tool_result in tool_results:
                    if item_dict.get("tool_call_id") == tool_result.tool_call_id:
                        updated_items.append({
                            ...
                            "content": tool_result.result or f"Tool {tool_result.tool_name} executed successfully"
                        })
                        logger.info(f"Replaced pending result for tool {tool_result.tool_name}")
                        break
            else:
        ...
        logger.info(f"Updated session with {len(updated_items)} items including actual tool results")

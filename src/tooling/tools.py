...
@tool_metadata(
    ToolMetadata(description="Get current time in any timezone with formatted output")
)
@function_tool
async def get_current_time(timezone: str = "UTC") -> str:
    ...
    try:
        ...
        logger.error(
            f"Error getting time for timezone {timezone}: {str(e)}", exc_info=True
        )
...
@tool_metadata(
    ToolMetadata(description="Calculate the difference between two dates in days")
)
@function_tool
async def date_difference(start_date: str, end_date: str) -> str:
    ...
    try:
        ...
        if days == 0:
            ...
            return (
                f"{abs(days)} days between {end_date} and {start_date} (dates reversed)"
            )
    except ValueError:
        logger.error("Invalid date format in date_difference")
        return "Invalid date format. Please use YYYY-MM-DD format (e.g., 2024-12-29)"
    except Exception as e:
...
async def unit_converter(value: float, from_unit: str, to_unit: str) -> str:
    ...
    try:
        ...
        if from_unit_lower in ["celsius", "fahrenheit", "kelvin"] and to_unit_lower in [
            "celsius",
            "fahrenheit",
            "kelvin",
        ]:
...
@tool_metadata(
    ToolMetadata(description="Convert currency amounts using real-time exchange rates")
)
@function_tool
async def currency_converter(
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            return (
                f"Error: Unable to fetch exchange rates (status {response.status_code})"
            )
        ...
        return "Error: Unable to connect to currency service"
...
async def get_stock_price(symbol: str) -> str:
    ...
    try:
        ...
        if api_key == "demo":
            ...
            logger.warning(
                "Using demo mode for stock prices. Set ALPHA_VANTAGE_API_KEY for real data."
            )
            mock_prices = {
                "AAPL": {
                    "price": 195.89,
...
"""
            ...
            result += (
                ...
                else f"Total Revenue: {revenue}\n"
            )
            result += (
                f"Net Income: ${profit:,.0f}\n"
                if isinstance(profit, (int, float))
                else f"Net Income: {profit}\n"
            )
            result += (
                f"EPS (TTM): ${eps:.2f}\n"
            ...
            result += (
                f"Profit Margin: {info.get('profitMargins', 'N/A'):.2%}\n"
            ...
            return "Invalid info_type. Choose from: 'summary', 'price', 'financials', 'holders', 'news'"
